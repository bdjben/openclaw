...
For each native message event, one deterministic local owner makes one shared, low-reasoning utility-model call for the whole room roster. The model receives bounded recent room context, the current message, routed agent identities, and bounded active sibling-preview progress. It returns exactly one disposition per candidate:
...
The decision is shared across account monitors, so one Matrix event does not create competing classifier calls. Invalid output, model failure, or timeout fails open to `neutral`. Commands, including `/steer`, bypass participation classification and continue through the normal command path.
